CREATE TABLE `workers` (`id` text,`last_heartbeat` datetime,`hostname` text,`type` text,PRIMARY KEY (`id`));

CREATE TABLE `globals` (`key` text,`value` text,PRIMARY KEY (`key`));

CREATE TABLE `preparations` (`id` integer,`name` text UNIQUE,`created_at` datetime,`updated_at` datetime,`delete_after_export` numeric,`max_size` integer,`piece_size` integer,`min_piece_size` integer,`no_inline` numeric,`no_dag` numeric,PRIMARY KEY (`id`));

CREATE TABLE `storages` (`id` integer,`name` text UNIQUE,`created_at` datetime,`updated_at` datetime,`type` text,`path` text,`config` JSON,`client_config` JSON,PRIMARY KEY (`id`));
