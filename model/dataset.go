...
// RootDirectory retrieves the root directory associated with a Storage entity.
// It queries the database to find the directory that has a source ID equal to the ID of the Storage entity
// and a NULL parent ID, indicating that it is the root directory.
//
// Parameters:
// ctx - The context to control the lifecycle of the database request.
// db  - The gorm.DB object representing the database session.
//
// Returns:
// - A pointer to a Directory struct representing the root directory if found.
// - An error if the root directory is not found or if any other error occurs while querying the database.
func (s *Storage) RootDirectory(ctx context.Context, db *gorm.DB) (*Directory, error) {
	var rootDir Directory
	db = db.WithContext(ctx)
	err := db.Where("source_id = ? AND parent_id IS NULL", s.ID).First(&rootDir).Error
	if err != nil {
		return nil, err
	}
...
type Directory struct {
	...
	// Associations
	PreparationSourceAttachmentID uint32                       `gorm:"index:directory_source_parent" json:"sourceStorageId"`
	PreparationSourceAttachment   *PreparationSourceAttachment `gorm:"foreignKey:SourceStorageID;constraint:OnDelete:CASCADE" json:"sourceStorage,omitempty" swaggerignore:"true"`
...
type FileRange struct {
	...
	Job    *Job    `gorm:"foreignKey:PackJobID;constraint:OnDelete:SET NULL" json:"job,omitempty" swaggerignore:"true"`
