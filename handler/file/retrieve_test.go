...
func TestRetrieveFileHandlerWithSeek(t *testing.T) {
	testCases := []struct {
		name          string
		keepLocalFile bool
	}{
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			testutil.All(t, func(ctx context.Context, t *testing.T, db *gorm.DB) {
				...
				require.Equal(t, expected, outBuf)
				if !testCase.keepLocalFile {
					require.Len(t, fr.requests, 1) // only one because 1st read was got leftover data already in rangeReader
					require.Equal(t, retrieveRequest{ranges[2].file.Root, 0, 1 << 20, []string{deals[2].Provider}}, fr.requests[0])
					fr.requests = nil
				}
				qtLen := len(outBuf) / 4
				_, err = seeker.Read(outBuf[:qtLen])
				require.NoError(t, err)
				_, err = seeker.Read(outBuf[qtLen : 2*qtLen])
				require.NoError(t, err)
				_, err = seeker.Read(outBuf[2*qtLen : 3*qtLen])
				require.NoError(t, err)

				// Replace "_, err = seeker.Read(outBuf[3*qtLen:])" with seek.
				_, err = seeker.Seek(int64(qtLen), io.SeekCurrent)
				require.NoError(t, err)

				expected = bytes.Join([][]byte{ranges[2].expectedBytes[1<<19 : 1<<20], ranges[3].expectedBytes[0 : 1<<19]}, nil)
				require.Equal(t, expected[:3*qtLen], outBuf[:3*qtLen])

				if !testCase.keepLocalFile {
					require.Len(t, fr.requests, 1) // only one because 1st read was leftover data already in rangeReader
					require.Equal(t, retrieveRequest{ranges[3].file.Root, 0, 1 << 20, []string{deals[2].Provider}}, fr.requests[0])
					fr.requests = nil
				}
