...
func TestListPiecesHandler(t *testing.T) {
	...
	require.NoError(t, err)
	result, err := ListPiecesHandler(ctx, db, 1)
	require.NoError(t, err)
...
func TestListPiecesHandler_NotFound(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = ListPiecesHandler(ctx, db, 2)
...
func TestAddPieceHandler(t *testing.T) {
	...
	t.Run("not_found", func(t *testing.T) {
		_, err := AddPieceHandler(ctx, db, 100, AddPieceRequest{
			PieceCID:  "",
			PieceSize: "",
			FilePath:  "",
			RootCID:   "",
	...
	t.Run("pieceCID invalid", func(t *testing.T) {
		_, err := AddPieceHandler(ctx, db, 1, AddPieceRequest{
			PieceCID:  "invalid",
			PieceSize: "32",
			FilePath:  "",
			RootCID:   "",
	...
	t.Run("pieceCID invalid", func(t *testing.T) {
		_, err := AddPieceHandler(ctx, db, 1, AddPieceRequest{
			PieceCID:  pack.EmptyFileCid.String(),
			PieceSize: "32",
			FilePath:  "",
			RootCID:   "",
		})
	...
	t.Run("success", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = util.WriteCarHeader(f, pack.EmptyFileCid)
		require.NoError(t, err)
		f.Close()
		c, err := AddPieceHandler(ctx, db, 1, AddPieceRequest{
			PieceCID:  "baga6ea4seaqchxeb6cwpiephnus27kplk7lku225rdhrsgb3ej4smaqwgop6wkq",
			PieceSize: "65536",
			FilePath:  filepath.Join(tmp, "a.car"),
			RootCID:   "",
