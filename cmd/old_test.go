...
func TestWalletCrud(t *testing.T) {
	...
	WithAllBackend(t, func(ctx context.Context, t *testing.T, db *gorm.DB) {
		...
		require.NoError(t, err)
		out, _, err := Run(ctx, "singularity wallet list ")
		require.NoError(t, err)
		...
		out, _, err = Run(ctx, "singularity dataset add-wallet test f0808055")
		require.NoError(t, err)
		require.Contains(t, out, "f0808055")
		out, _, err = Run(ctx, "singularity dataset list-wallet test")
		require.NoError(t, err)
		require.Contains(t, out, "f0808055")
...
func TestDatasetAddPiece(t *testing.T) {
	WithAllBackend(t, func(ctx context.Context, t *testing.T, db *gorm.DB) {
		...
		require.NoError(t, err)
		out, _, err := Run(ctx, "singularity dataset list-pieces test")
		require.NoError(t, err)
...
func TestDatasetCrud(t *testing.T) {
	WithAllBackend(t, func(ctx context.Context, t *testing.T, db *gorm.DB) {
		tmp := t.TempDir()
		out, _, err := Run(ctx, "singularity dataset create test")
		require.NoError(t, err)
		require.Contains(t, out, "test")
		out, _, err = Run(ctx, "singularity dataset list")
		require.NoError(t, err)
		require.Contains(t, out, "test")
		out, _, err = Run(ctx, "singularity dataset update --output-dir "+escape(tmp)+" --max-size 1000 test")
		require.NoError(t, err)
...
func TestDatasourceCrud(t *testing.T) {
	WithAllBackend(t, func(ctx context.Context, t *testing.T, db *gorm.DB) {
		...
		_, _, err = Run(ctx, "singularity dataset create test")
		require.NoError(t, err)
		out, _, err := Run(ctx, "singularity datasource add local test "+escape(temp))
		require.NoError(t, err)
		require.Contains(t, out, temp)
		out, _, err = Run(ctx, "singularity datasource list")
		require.NoError(t, err)
		require.Contains(t, out, temp)
		out, _, err = Run(ctx, "singularity datasource list --dataset test")
		require.NoError(t, err)
		require.Contains(t, out, temp)
		_, _, err = Run(ctx, "singularity datasource list --dataset notexist")
		require.Error(t, err)
		out, _, err = Run(ctx, "singularity datasource check 1")
		require.NoError(t, err)
		require.Contains(t, out, "sub")
		out, _, err = Run(ctx, "singularity datasource check 1 sub")
		require.NoError(t, err)
		require.Contains(t, out, "sub/test.txt")
		out, _, err = Run(ctx, "singularity datasource update --local-case-sensitive=true --rescan-interval 1h 1")
		require.NoError(t, err)
...
func TestEncryption(t *testing.T) {
	WithAllBackend(t, func(ctx context.Context, t *testing.T, db *gorm.DB) {
		...
		require.NoError(t, err)
		// Get the root CID
		out, _, err := Run(ctx, "singularity --json datasource inspect path 1")
		require.NoError(t, err)
...
func TestDatasourceRescan(t *testing.T) {
	WithAllBackend(t, func(ctx context.Context, t *testing.T, db *gorm.DB) {
		...
		require.NoError(t, err)
		out, _, err := Run(ctx, "singularity datasource inspect packjobs 1")
		require.NoError(t, err)
		...
		out, _, err = Run(ctx, "singularity datasource inspect files 1")
		require.NoError(t, err)
		require.Contains(t, out, "baf")
		require.Contains(t, out, "test5.txt")
		out, _, err = Run(ctx, "singularity datasource inspect packjobdetail 1")
		require.NoError(t, err)
		require.Contains(t, out, "sub/test1.txt")
		require.Contains(t, out, "sub/test3.txt")
		out, _, err = Run(ctx, "singularity datasource inspect path 1")
		require.NoError(t, err)
		require.Contains(t, out, "sub")
		out, _, err = Run(ctx, "singularity datasource inspect path 1 sub/")
		require.NoError(t, err)
		require.Contains(t, out, "sub/test1.txt")
		require.Contains(t, out, "sub/test3.txt")
		out, _, err = Run(ctx, "singularity datasource daggen 1")
		require.NoError(t, err)
		require.Contains(t, out, "ready")
