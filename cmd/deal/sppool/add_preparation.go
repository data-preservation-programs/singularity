package sppool

import (
	"strconv"

	"github.com/cockroachdb/errors"
	"github.com/data-preservation-programs/singularity/cmd/cliutil"
	"github.com/data-preservation-programs/singularity/database"
	"github.com/data-preservation-programs/singularity/handler/deal/sppool"
	"github.com/urfave/cli/v2"
)

var AddPreparationCmd = &cli.Command{
	Name:      "add-preparation",
	Usage:     "Add a preparation to an SP Pool",
	Before:    cliutil.CheckNArgs,
	ArgsUsage: "<pool_id>",
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:     "preparation",
			Usage:    "Preparation ID or name",
			Required: true,
		},
	},
	Action: func(c *cli.Context) error {
		db, closer, err := database.OpenFromCLI(c)
		if err != nil {
			return errors.WithStack(err)
		}
		defer closer.Close()

		poolID, err := strconv.ParseUint(c.Args().Get(0), 10, 32)
		if err != nil {
			return errors.Wrapf(err, "failed to parse pool ID %s", c.Args().Get(0))
		}

		request := sppool.AddPreparationRequest{
			Preparation: c.String("preparation"),
		}
		prep, err := sppool.Default.AddPreparationHandler(c.Context, db, uint32(poolID), request)
		if err != nil {
			return errors.WithStack(err)
		}
		cliutil.Print(c, prep)
		return nil
	},
}
