...
- Filter by deal ID, state, provider, client address, and time range
- Support for pagination and sorting
- Export results to CSV or JSON formats`,
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:  "deal-id",
			Usage: "Filter by specific deal ID",
		},
		&cli.StringFlag{
			Name:  "state",
	...
	Action: func(c *cli.Context) error {
		...
		if startTimeStr := c.String("start-time"); startTimeStr != "" {
			...
			if err != nil {
				return errors.Wrap(err, "invalid start-time format, expected RFC3339 (e.g., 2023-01-01T00:00:00Z)")
			}
		...
		if endTimeStr := c.String("end-time"); endTimeStr != "" {
			...
			if err != nil {
				return errors.Wrap(err, "invalid end-time format, expected RFC3339 (e.g., 2023-12-31T23:59:59Z)")
			}
		...
		if exportFormat != "" {
			...
			err = exportStateChanges(response.StateChanges, exportFormat, outputPath)
			if err != nil {
				return errors.WithStack(err)
			}
