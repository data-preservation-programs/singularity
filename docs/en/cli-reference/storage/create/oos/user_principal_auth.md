...
```
NAME:
   singularity storage create oos user_principal_auth - use an OCI user and an API key for authentication.
                                                        you’ll need to put in a config file your tenancy OCID, user OCID, region, the path, fingerprint to an API key.
                                                        https://docs.oracle.com/en-us/iaas/Content/API/Concepts/sdkconfig.htm

USAGE:
   singularity storage create oos user_principal_auth [command options] <name> <path>
...
OPTIONS:
   ...
   --config_file value     Path to OCI config file (default: "~/.oci/config") [$CONFIG_FILE]
   --config_profile value  Profile name inside the oci config file (default: "Default") [$CONFIG_PROFILE]
