...
OPTIONS:
   ...
   --bucket_acl value                   Access Control List for new buckets. [$BUCKET_ACL]
   --bucket_policy_only                 Access checks should use bucket-level IAM policies. (default: false) [$BUCKET_POLICY_ONLY]
   --client_id value                    OAuth Client Id. [$CLIENT_ID]
   --client_secret value                OAuth Client Secret. [$CLIENT_SECRET]
   --env_auth                           Get GCP IAM credentials from runtime (environment variables or instance meta data if no env vars). (default: false) [$ENV_AUTH]
   --help, -h                           show help
   --location value                     Location for the newly created buckets. [$LOCATION]
   --object_acl value                   Access Control List for new objects. [$OBJECT_ACL]
   --project_number value               Project number. [$PROJECT_NUMBER]
   ...
   --storage_class value                The storage class to use when storing objects in Google Cloud Storage. [$STORAGE_CLASS]
