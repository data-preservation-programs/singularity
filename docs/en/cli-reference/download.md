...
OPTIONS:
   ...
   --s3-decompress                     If set this will decompress gzip encoded objects. (default: false) [$S3_DECOMPRESS]
   --s3-disable-checksum               Don't store MD5 checksum with object metadata. (default: false) [$S3_DISABLE_CHECKSUM]
   --s3-disable-http2                  Disable usage of http2 for S3 backends. (default: false) [$S3_DISABLE_HTTP2]
   ...
   --s3-env-auth                       Get AWS credentials from runtime (environment variables or EC2/ECS meta data if no env vars). (default: false) [$S3_ENV_AUTH]
   --s3-force-path-style               If true use path style access if false use virtual hosted style. (default: true) [$S3_FORCE_PATH_STYLE]
   --s3-leave-parts-on-error           If true avoid calling abort upload on a failure, leaving all successfully uploaded parts on S3 for manual recovery. (default: false) [$S3_LEAVE_PARTS_ON_ERROR]
   ...
   --s3-memory-pool-use-mmap           Whether to use mmap buffers in internal memory pool. (default: false) [$S3_MEMORY_POOL_USE_MMAP]
   --s3-might-gzip value               Set this if the backend might gzip objects. (default: "unset") [$S3_MIGHT_GZIP]
   --s3-no-check-bucket                If set, don't attempt to check the bucket exists or create it. (default: false) [$S3_NO_CHECK_BUCKET]
   --s3-no-head                        If set, don't HEAD uploaded objects to check integrity. (default: false) [$S3_NO_HEAD]
   --s3-no-head-object                 If set, do not do HEAD before GET when getting objects. (default: false) [$S3_NO_HEAD_OBJECT]
   --s3-no-system-metadata             Suppress setting and reading of system metadata (default: false) [$S3_NO_SYSTEM_METADATA]
   --s3-profile value                  Profile to use in the shared credentials file. [$S3_PROFILE]
   --s3-provider value                 AWS | Alibaba | ArvanCloud | Ceph | ChinaMobile | Cloudflare | DigitalOcean | Dreamhost | HuaweiOBS | IBMCOS | IDrive | IONOS | Liara | LyveCloud | Minio | Netease | Other | Qiniu | RackCorp | Scaleway | SeaweedFS | StackPath | Storj | TencentCOS | Wasabi (default: "AWS") [$S3_PROVIDER]
   --s3-region value                   Region to connect to. [$S3_REGION]
   --s3-requester-pays                 Enables requester pays option when interacting with S3 bucket. (default: false) [$S3_REQUESTER_PAYS]
   ...
   --s3-use-accelerate-endpoint        If true use the AWS S3 accelerated endpoint. (default: false) [$S3_USE_ACCELERATE_ENDPOINT]
   ...
   --s3-use-presigned-request          Whether to use a presigned request or PutObject for single part uploads (default: false) [$S3_USE_PRESIGNED_REQUEST]
   --s3-v2-auth                        If true use v2 authentication. (default: false) [$S3_V2_AUTH]
   --s3-version-at value               Show file versions as they were at the specified time. (default: "off") [$S3_VERSION_AT]
   --s3-versions                       Include old versions in directory listings. (default: false) [$S3_VERSIONS]
   ...
   --b2-disable-checksum              Disable checksums for large (> upload cutoff) files. (default: false) [$B2_DISABLE_CHECKSUM]
   ...
   --b2-hard-delete                   Permanently delete files on remote removal, otherwise hide files. (default: false) [$B2_HARD_DELETE]
   ...
   --b2-memory-pool-use-mmap          Whether to use mmap buffers in internal memory pool. (default: false) [$B2_MEMORY_POOL_USE_MMAP]
   ...
   --dropbox-shared-files                Instructs rclone to work on individual shared files. (default: false) [$DROPBOX_SHARED_FILES]
   --dropbox-shared-folders              Instructs rclone to work on shared folders. (default: false) [$DROPBOX_SHARED_FOLDERS]
   ...
   --ftp-disable-epsv          Disable using EPSV even if server advertises support. (default: false) [$FTP_DISABLE_EPSV]
   --ftp-disable-mlsd          Disable using MLSD even if server advertises support. (default: false) [$FTP_DISABLE_MLSD]
   --ftp-disable-tls13         Disable TLS 1.3 (workaround for FTP servers with buggy TLS) (default: false) [$FTP_DISABLE_TLS13]
   --ftp-disable-utf8          Disable using UTF-8 even if server advertises support. (default: false) [$FTP_DISABLE_UTF8]
   --ftp-encoding value        The encoding for the backend. (default: "Slash,Del,Ctl,RightSpace,Dot") [$FTP_ENCODING]
   --ftp-explicit-tls          Use Explicit FTPS (FTP over TLS). (default: false) [$FTP_EXPLICIT_TLS]
   --ftp-force-list-hidden     Use LIST -a to force listing of hidden files and folders. This will disable the use of MLSD. (default: false) [$FTP_FORCE_LIST_HIDDEN]
   ...
   --ftp-no-check-certificate  Do not verify the TLS certificate of the server. (default: false) [$FTP_NO_CHECK_CERTIFICATE]
   ...
   --ftp-writing-mdtm          Use MDTM to set modification time (VsFtpd quirk) (default: false) [$FTP_WRITING_MDTM]
   ...
   --gcs-bucket-policy-only                 Access checks should use bucket-level IAM policies. (default: false) [$GCS_BUCKET_POLICY_ONLY]
   ...
   --gcs-env-auth                           Get GCP IAM credentials from runtime (environment variables or instance meta data if no env vars). (default: false) [$GCS_ENV_AUTH]
   --gcs-location value                     Location for the newly created buckets. [$GCS_LOCATION]
   --gcs-no-check-bucket                    If set, don't attempt to check the bucket exists or create it. (default: false) [$GCS_NO_CHECK_BUCKET]
   ...
   --gcs-service-account-credentials value  Service Account Credentials JSON blob. [$GCS_SERVICE_ACCOUNT_CREDENTIALS]
   ...
   --drive-acknowledge-abuse                  Set to allow files which return cannotDownloadAbusiveFile to be downloaded. (default: false) [$DRIVE_ACKNOWLEDGE_ABUSE]
   --drive-allow-import-name-change           Allow the filetype to change when uploading Google docs. (default: false) [$DRIVE_ALLOW_IMPORT_NAME_CHANGE]
   --drive-alternate-export                   Deprecated: No longer needed. (default: false) [$DRIVE_ALTERNATE_EXPORT]
   --drive-auth-owner-only                    Only consider files owned by the authenticated user. (default: false) [$DRIVE_AUTH_OWNER_ONLY]
   ...
   --drive-copy-shortcut-content              Server side copy contents of shortcuts instead of the shortcut. (default: false) [$DRIVE_COPY_SHORTCUT_CONTENT]
   --drive-disable-http2                      Disable drive using http2. (default: true) [$DRIVE_DISABLE_HTTP2]
   ...
   --drive-keep-revision-forever              Keep new head revision of each file forever. (default: false) [$DRIVE_KEEP_REVISION_FOREVER]
   ...
   --drive-server-side-across-configs         Allow server-side operations (e.g. copy) to work across different drive configs. (default: false) [$DRIVE_SERVER_SIDE_ACROSS_CONFIGS]
   ...
   --drive-shared-with-me                     Only show files that are shared with me. (default: false) [$DRIVE_SHARED_WITH_ME]
   --drive-size-as-quota                      Show sizes as storage quota usage, not actual size. (default: false) [$DRIVE_SIZE_AS_QUOTA]
   --drive-skip-checksum-gphotos              Skip MD5 checksum on Google photos and videos only. (default: false) [$DRIVE_SKIP_CHECKSUM_GPHOTOS]
   --drive-skip-dangling-shortcuts            If set skip dangling shortcut files. (default: false) [$DRIVE_SKIP_DANGLING_SHORTCUTS]
   --drive-skip-gdocs                         Skip google documents in all listings. (default: false) [$DRIVE_SKIP_GDOCS]
   --drive-skip-shortcuts                     If set skip shortcut files. (default: false) [$DRIVE_SKIP_SHORTCUTS]
   --drive-starred-only                       Only show files that are starred. (default: false) [$DRIVE_STARRED_ONLY]
   --drive-stop-on-download-limit             Make download limit errors be fatal. (default: false) [$DRIVE_STOP_ON_DOWNLOAD_LIMIT]
   --drive-stop-on-upload-limit               Make upload limit errors be fatal. (default: false) [$DRIVE_STOP_ON_UPLOAD_LIMIT]
   ...
   --drive-trashed-only                       Only show files that are in the trash. (default: false) [$DRIVE_TRASHED_ONLY]
   --drive-upload-cutoff value                Cutoff for switching to chunked upload. (default: "8Mi") [$DRIVE_UPLOAD_CUTOFF]
   --drive-use-created-date                   Use file created date instead of modified date. (default: false) [$DRIVE_USE_CREATED_DATE]
   --drive-use-shared-date                    Use date file was shared instead of modified date. (default: false) [$DRIVE_USE_SHARED_DATE]
   --drive-use-trash                          Send files to the trash instead of deleting permanently. (default: true) [$DRIVE_USE_TRASH]
   ...
   --gphotos-include-archived     Also view and download archived media. (default: false) [$GPHOTOS_INCLUDE_ARCHIVED]
   --gphotos-read-only            Set to make the Google Photos backend read only. (default: false) [$GPHOTOS_READ_ONLY]
   --gphotos-read-size            Set to read the size of media items. (default: false) [$GPHOTOS_READ_SIZE]
   ...
   --hidrive-disable-fetching-member-count  Do not fetch number of objects in directories unless it is absolutely necessary. (default: false) [$HIDRIVE_DISABLE_FETCHING_MEMBER_COUNT]
   ...
   --internetarchive-disable-checksum         Don't ask the server to test against MD5 checksum calculated by rclone. (default: true) [$INTERNETARCHIVE_DISABLE_CHECKSUM]
   ...
   --jottacloud-hard-delete                Delete files permanently rather than putting them into the trash. (default: false) [$JOTTACLOUD_HARD_DELETE]
   ...
   --jottacloud-no-versions                Avoid server side versioning by deleting files and recreating files instead of overwriting them. (default: false) [$JOTTACLOUD_NO_VERSIONS]
   --jottacloud-trashed-only               Only show files that are in the trash. (default: false) [$JOTTACLOUD_TRASHED_ONLY]
   ...
   --koofr-provider value  digistorage | koofr | other (default: "digistorage") [$KOOFR_PROVIDER]
   ...
   --local-case-insensitive       Force the filesystem to report itself as case insensitive. (default: false) [$LOCAL_CASE_INSENSITIVE]
   --local-case-sensitive         Force the filesystem to report itself as case sensitive. (default: false) [$LOCAL_CASE_SENSITIVE]
   --local-copy-links             Follow symlinks and copy the pointed to item. (default: false) [$LOCAL_COPY_LINKS]
   ...
   --local-no-check-updated       Don't check to see if the files change during upload. (default: false) [$LOCAL_NO_CHECK_UPDATED]
   --local-no-preallocate         Disable preallocation of disk space for transferred files. (default: false) [$LOCAL_NO_PREALLOCATE]
   --local-no-set-modtime         Disable setting modtime. (default: false) [$LOCAL_NO_SET_MODTIME]
   --local-no-sparse              Disable sparse files for multi-thread downloads. (default: false) [$LOCAL_NO_SPARSE]
   --local-nounc                  Disable UNC (long path names) conversion on Windows. (default: false) [$LOCAL_NOUNC]
   --local-one-file-system        Don't cross filesystem boundaries (unix/macOS only). (default: false) [$LOCAL_ONE_FILE_SYSTEM]
   --local-skip-links             Don't warn about skipped symlinks. (default: false) [$LOCAL_SKIP_LINKS]
   --local-unicode-normalization  Apply unicode NFC normalization to paths and filenames. (default: false) [$LOCAL_UNICODE_NORMALIZATION]
   --local-zero-size-links        Assume the Stat size of links is zero (and read them instead) (deprecated). (default: false) [$LOCAL_ZERO_SIZE_LINKS]

   Mail.ru Cloud

   --mailru-check-hash                   What should copy do if file checksum is mismatched or invalid. (default: true) [$MAILRU_CHECK_HASH]
   ...
   --mailru-speedup-enable               Skip full upload if there is another file with same data hash. (default: true) [$MAILRU_SPEEDUP_ENABLE]
   ...
   --mailru-user-agent value             HTTP user agent used internally by client. [$MAILRU_USER_AGENT]
   ...
   --mega-hard-delete     Delete files permanently rather than putting them into the trash. (default: false) [$MEGA_HARD_DELETE]
   --mega-pass value      Password. [$MEGA_PASS]
   --mega-use-https       Use HTTPS for transfers. (default: false) [$MEGA_USE_HTTPS]
   ...
   --azureblob-archive-tier-delete                Delete archive tier blobs before overwriting. (default: false) [$AZUREBLOB_ARCHIVE_TIER_DELETE]
   ...
   --azureblob-client-send-certificate-chain      Send the certificate chain when using certificate auth. (default: false) [$AZUREBLOB_CLIENT_SEND_CERTIFICATE_CHAIN]
   --azureblob-disable-checksum                   Don't store MD5 checksum with object metadata. (default: false) [$AZUREBLOB_DISABLE_CHECKSUM]
   ...
   --azureblob-env-auth                           Read credentials from runtime (environment variables, CLI or MSI). (default: false) [$AZUREBLOB_ENV_AUTH]
   ...
   --azureblob-memory-pool-use-mmap               Whether to use mmap buffers in internal memory pool. (default: false) [$AZUREBLOB_MEMORY_POOL_USE_MMAP]
   ...
   --azureblob-no-check-container                 If set, don't attempt to check the container exists or create it. (default: false) [$AZUREBLOB_NO_CHECK_CONTAINER]
   --azureblob-no-head-object                     If set, do not do HEAD before GET when getting objects. (default: false) [$AZUREBLOB_NO_HEAD_OBJECT]
   ...
   --azureblob-use-emulator                       Uses local storage emulator if provided as 'true'. (default: false) [$AZUREBLOB_USE_EMULATOR]
   --azureblob-use-msi                            Use a managed service identity to authenticate (only works in Azure). (default: false) [$AZUREBLOB_USE_MSI]
   ...
   --onedrive-disable-site-permission     Disable the request for Sites.Read.All permission. (default: false) [$ONEDRIVE_DISABLE_SITE_PERMISSION]
   ...
   --onedrive-expose-onenote-files        Set to make OneNote files show up in directory listings. (default: false) [$ONEDRIVE_EXPOSE_ONENOTE_FILES]
   ...
   --onedrive-no-versions                 Remove all versions on modifying operations. (default: false) [$ONEDRIVE_NO_VERSIONS]
   ...
   --onedrive-server-side-across-configs  Allow server-side operations (e.g. copy) to work across different onedrive configs. (default: false) [$ONEDRIVE_SERVER_SIDE_ACROSS_CONFIGS]
   ...
   --swift-env-auth                             Get swift credentials from environment variables in standard OpenStack form. (default: false) [$SWIFT_ENV_AUTH]
   --swift-key value                            API key or password (OS_PASSWORD). [$SWIFT_KEY]
   --swift-leave-parts-on-error                 If true avoid calling abort upload on a failure. (default: false) [$SWIFT_LEAVE_PARTS_ON_ERROR]
   --swift-no-chunk                             Don't chunk files during streaming upload. (default: false) [$SWIFT_NO_CHUNK]
   --swift-no-large-objects                     Disable support for static and dynamic large objects (default: false) [$SWIFT_NO_LARGE_OBJECTS]
   ...
   --oos-disable-checksum               Don't store MD5 checksum with object metadata. (default: false) [$OOS_DISABLE_CHECKSUM]
   ...
   --oos-leave-parts-on-error           If true avoid calling abort upload on a failure, leaving all successfully uploaded parts on S3 for manual recovery. (default: false) [$OOS_LEAVE_PARTS_ON_ERROR]
   --oos-namespace value                Object storage namespace [$OOS_NAMESPACE]
   --oos-no-check-bucket                If set, don't attempt to check the bucket exists or create it. (default: false) [$OOS_NO_CHECK_BUCKET]
   --oos-provider value                 env_auth | instance_principal_auth | no_auth | resource_principal_auth | user_principal_auth (default: "env_auth") [$OOS_PROVIDER]
   ...
   --qingstor-env-auth                  Get QingStor credentials from runtime. (default: false) [$QINGSTOR_ENV_AUTH]
   ...
   --smb-hide-special-share  Hide special shares (e.g. print$) which users aren't supposed to access. (default: true) [$SMB_HIDE_SPECIAL_SHARE]
   ...
   --sftp-disable-concurrent-reads   If set don't use concurrent reads. (default: false) [$SFTP_DISABLE_CONCURRENT_READS]
   --sftp-disable-concurrent-writes  If set don't use concurrent writes. (default: false) [$SFTP_DISABLE_CONCURRENT_WRITES]
   --sftp-disable-hashcheck          Disable the execution of SSH commands to determine if remote file hashing is available. (default: false) [$SFTP_DISABLE_HASHCHECK]
   ...
   --sftp-set-modtime                Set the modified time on the remote if set. (default: true) [$SFTP_SET_MODTIME]
   ...
   --sftp-skip-links                 Set to skip any symlinks and any other non regular files. (default: false) [$SFTP_SKIP_LINKS]
   --sftp-subsystem value            Specifies the SSH2 subsystem on the remote host. (default: "sftp") [$SFTP_SUBSYSTEM]
   --sftp-use-fstat                  If set use fstat instead of stat. (default: false) [$SFTP_USE_FSTAT]
   --sftp-use-insecure-cipher        Enable the use of insecure ciphers and key exchange methods. (default: false) [$SFTP_USE_INSECURE_CIPHER]
   ...
   --storj-provider value           existing | new (default: "existing") [$STORJ_PROVIDER]
   ...
   --sugarsync-hard-delete                 Permanently delete files if true (default: false) [$SUGARSYNC_HARD_DELETE]
   ...
   --yandex-hard-delete          Delete files permanently rather than putting them into the trash. (default: false) [$YANDEX_HARD_DELETE]
   ...
   --seafile-create-library     Should rclone create a library if it doesn't exist. (default: false) [$SEAFILE_CREATE_LIBRARY]
