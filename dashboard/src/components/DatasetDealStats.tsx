...
interface DatasetDealStatsProps {
  datasetId: string;
}
...
const DatasetDealStats: React.FC<DatasetDealStatsProps> = ({ datasetId }) => {
  const [dealStats, setDealStats] = useState<DealStats[]>([]);

  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/dataset/${datasetId}/deal_stats`).then((response) => {
...
export default DatasetDealStats;
